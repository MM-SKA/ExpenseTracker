...
public class CategoryService : ICategoryService
{
    ...
    {
        var category = await _context.Categories.FirstOrDefaultAsync(c => c.Id == categoryId && (c.IsSystemCategory || c.UserId == userId));
        if (category == null)
        {
        ...
        var isDuplicate = await _context.Categories.AnyAsync(c => (c.IsSystemCategory || c.UserId == userId) && c.Name.ToLower().Trim() == request.Name.ToLower().Trim() && c.Id != categoryId);
        if (isDuplicate)
        {
            return new ApiResponse<CategoryDto> { Success = false, Message = "Category with this name already exists" };
        }
