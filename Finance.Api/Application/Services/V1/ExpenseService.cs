    ...
    : IExpenseServiceV1<ExpenseDtoV1>
{
    ...
    CancellationToken cancellationToken)
    {
        ...
        var totalRecords = await EntityFrameworkQueryableExtensions.CountAsync(query, cancellationToken).ConfigureAwait(false);
        ...
        query = filters.SortOrder.Equals("oldest", StringComparison.OrdinalIgnoreCase) ? query.OrderBy(e => e.ExpenseDate) : query.OrderByDescending(e => e.ExpenseDate);
        ...
        {
            Items = [.. expenses.Select(e => new ExpenseDtoV1{
                Id = e.Id,
                Description = e.Notes ?? string.Empty,
                Amount = e.Amount,
                Date = e.ExpenseDate,
    ...
    CancellationToken cancellationToken)
    {
        ...
        var worksheet =
            workbook.Worksheets.Add("Expenses");
        ...
        foreach (var expense in expenses)
        {
            ...
            worksheet.Cell(row, 2).Value =
                expense.CategoryName ?? string.Empty;
            ...
            worksheet.Cell(row, 5).Value =
                expense.Location ?? string.Empty;

            worksheet.Cell(row, 3).Style.NumberFormat.Format =
                "₹ #,##0.00";

            worksheet.Cell(row, 4).Style.DateFormat.Format =
                "dd MMM yyyy";
